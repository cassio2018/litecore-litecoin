...
#// full deployement :   wget -O - https://raw.githubusercontent.com/badbrainIRC/litecoin/master/go.sh | bash
sudo rm -Rf ~/litecoin
...
sudo add-apt-repository -y ppa:litecoin/litecoin
...
git clone --recurse-submodules https://github.com/litecoin/litecoin.git
cd ~
cd litecoin
cd ~/src/litecoin
find . -type f -print0 | xargs -0 sed -i 's/litecoin/litecoinevolution/g'
find . -type f -print0 | xargs -0 sed -i 's/Litecoin/litecoinEvolution/g'
find . -type f -print0 | xargs -0 sed -i 's/LiteCoin/litecoinEvolution/g'
find . -type f -print0 | xargs -0 sed -i 's/LITECOIN/litecoinEVOLUITON/g'
find . -type f -print0 | xargs -0 sed -i 's/LTC/BTCev/g'
./autogen.sh
chmod 777 ~/litecoin/share/genbuild.sh
chmod 777 ~/litecoin/src/leveldb/build_detect_platform
...
sed -i 's/#include <emmintrin.h>//g' ~/litecoinevolution/src/crypto/pow/scrypt-sse2.cpp
else
txt=$(echo "")
fi
...
if [ -e ~/litecoinevolution/src/qt/litecoin-qt ]
then
 strip ~/litecoinevolution/src/litecoind
 strip ~/litecoinevolution/src/qt/litecoin-qt
 make install
else
echo "Compile fail not litecoin-qt present"
fi
